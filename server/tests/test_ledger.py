...
def test_manual_expense_crud_and_month_total(client: TestClient) -> None:
    ...
    r = client.post("/api/ledger/expenses", json={
        "user_id": uid, "amount_fen": 3550, "category": "餐饮", "merchant": "麦当劳"})
    assert r.status_code == 200 and r.json()["status"] == "confirmed"
    e1 = r.json()["id"]
    # 收入记负数；不认得的分类归「其他」
    r = client.post("/api/ledger/expenses", json={
        "user_id": uid, "amount_fen": -5000, "category": "工资外快", "note": "闲鱼出货"})
    assert r.status_code == 200
    e2 = r.json()["id"]

    body = client.get("/api/ledger/expenses", params={"user_id": uid, "month": MONTH}).json()
    ...
    assert client.post("/api/ledger/expenses", json={"user_id": uid}).status_code == 400
    assert client.post(
        "/api/ledger/expenses", json={"user_id": uid, "amount_fen": 0}).status_code == 400
    assert client.post(
        "/api/ledger/expenses", json={"user_id": uid, "amount_fen": 10.5}).status_code == 422
    assert client.post("/api/ledger/expenses", json={
        "user_id": uid, "amount_fen": 100, "spent_at": "昨天"}).status_code == 400
    assert client.get(
        "/api/ledger/expenses", params={"user_id": uid, "month": "2026-8"}).status_code == 400

    # 改账：传什么改什么；越权 403
    r = client.put(f"/api/ledger/expenses/{e1}", json={"user_id": uid, "amount_fen": 3600})
    assert r.status_code == 200
    assert client.put(
        f"/api/ledger/expenses/{e1}", json={"user_id": other, "amount_fen": 1}).status_code == 403
    body = client.get("/api/ledger/expenses", params={"user_id": uid, "month": MONTH}).json()
    assert body["month_total_fen"] == 3600

    # 删账：越权 403；owner 删完列表与合计都缩回去
    assert client.delete(
        f"/api/ledger/expenses/{e1}", params={"user_id": other}).status_code == 403
    assert client.delete(f"/api/ledger/expenses/{e1}", params={"user_id": uid}).status_code == 200
    body = client.get("/api/ledger/expenses", params={"user_id": uid, "month": MONTH}).json()
...
def test_recognize_400_in_mock_mode(client: TestClient) -> None:
    ...
    url = _image_url()
    r = client.post("/api/ledger/recognize", json={"user_id": uid, "image_url": url})
    assert r.status_code == 400 and "手动录入" in r.json()["detail"]
    r = client.post("/api/calories/recognize", json={"user_id": uid, "image_url": url})
    assert r.status_code == 400 and "手动录入" in r.json()["detail"]
    # 站外地址直接 400（不泄露任何信息）
    assert client.post("/api/ledger/recognize", json={
        "user_id": uid, "image_url": "https://evil.com/x.jpg"}).status_code == 400
    body = client.get("/api/ledger/expenses", params={"user_id": uid, "month": MONTH}).json()
...
def test_recognize_then_confirm_expense_flow(client: TestClient, monkeypatch) -> None:
    ...
    r = client.post("/api/ledger/recognize", json={"user_id": uid, "image_url": _image_url()})
    ...
    r = client.post("/api/ledger/expenses", json={
        "user_id": uid, "id": items[0]["id"], "amount_fen": 3600, "spent_at": TODAY})
    assert r.status_code == 200 and r.json()["status"] == "confirmed"
    client.post("/api/ledger/expenses", json={"user_id": uid, "id": items[1]["id"], "spent_at": TODAY})
    ...
    # 重复确认幂等：再确认一次金额不被改写
    r = client.post("/api/ledger/expenses", json={
        "user_id": uid, "id": items[0]["id"], "amount_fen": 9999})
    assert r.status_code == 200 and r.json()["status"] == "confirmed"
    body = client.get("/api/ledger/expenses", params={"user_id": uid, "month": MONTH}).json()
...
def test_recognize_then_confirm_calorie_flow(client: TestClient, monkeypatch) -> None:
    ...
    r = client.post("/api/calories/recognize", json={
        "user_id": uid, "image_url": _image_url(), "hint": "家常一份"})
    assert r.status_code == 200, r.text
    entry = r.json()["entry"]
    ...
    # pending 不进当日清单
    body = client.get("/api/calories", params={"user_id": uid, "date": TODAY}).json()
    assert body["items"] == [] and body["consumed_kcal"] == 0

    # 确认时改成 500 kcal：等效按 500、默认体重 65kg 重算
    r = client.post("/api/calories", json={"user_id": uid, "id": entry["id"], "total_kcal": 500})
    assert r.status_code == 200 and r.json()["status"] == "confirmed"
    ...
    # 重复确认幂等：数字不再被改
    client.post("/api/calories", json={"user_id": uid, "id": entry["id"], "total_kcal": 9999})
    assert client.get("/api/calories", params={"user_id": uid, "date": TODAY}).json()["consumed_kcal"] == 500
...
def _weight_loss_goal_with_budget(client: TestClient, uid: str, budget_kcal: int) -> str:
    ...
    r = client.post("/api/goals", json={
        "user_id": uid, "type": "weight_loss", "title": "减重", "answers": {"weight_kg": 70}})
    assert r.status_code == 200, r.text
    gid = r.json()["id"]
    conn = _db()
...
def test_calorie_over_budget_adjust_item(client: TestClient) -> None:
    ...
    r = client.post("/api/calories", json={"user_id": uid, "total_kcal": 1000, "note": "早饭"})
    ...
    pending_id = conn.execute(
        "SELECT id FROM calorie_entries WHERE user_id = ? AND status = 'pending'", (uid,)
    ).fetchone()["id"]
    conn.close()
    r = client.post("/api/calories", json={"user_id": uid, "id": pending_id})
    assert r.status_code == 200, r.text
    ...
    r = client.post("/api/calories", json={"user_id": uid, "total_kcal": 500, "note": "下午茶"})
...
def test_calorie_no_goal_no_linkage(client: TestClient) -> None:
    ...
    r = client.post("/api/calories", json={"user_id": uid, "total_kcal": 9000})
    assert r.status_code == 200 and r.json()["adjustment"] is None
    assert _adjust_items(client, uid) == []
    body = client.get("/api/calories", params={"user_id": uid, "date": TODAY}).json()
...
def test_savings_monthly_settlement_lazy(client: TestClient) -> None:
    ...
    r = client.post("/api/goals", json={
        "user_id": uid, "type": "savings", "title": "攒旅行基金",
    ...
    client.post("/api/ledger/expenses", json={
        "user_id": uid, "amount_fen": 300000, "category": "餐饮", "spent_at": f"{m1}-15"})
    client.post("/api/ledger/expenses", json={
        "user_id": uid, "amount_fen": -100000, "category": "其他", "spent_at": f"{m1}-16"})
    ...
    d = client.get(f"/api/goals/{gid}", params={"viewer_id": uid}).json()
    ...
    d2 = client.get(f"/api/goals/{gid}", params={"viewer_id": uid}).json()
