...
**LangGraph 状态图（六节点；2026-08-22 提速改造后为五节点，护栏前置）**：

```
用户消息
  → ① 意图路由+查询改写（一次 LLM 调用合并出 intent 与检索 query，原两步串行合并）：
       倾诉 / 提问 / 需要查数据（决定走不走③）；倾诉不做改写
       ★ 护栏前置（原⑤生成后检的改造）：命中强烈自伤意愿 → 直接产出干预话术，
         条件边跳⑥写回——检索/工具/生成整段省掉（检查对象本就是用户消息，
         没必要等 120s 生成完再替换）
  → ② 检索（RAG 的 R+A）：
       - query 来自①；倾诉轮用原句做共鸣检索
       - 混合检索：向量余弦 + LIKE bigram 双路，RRF 融合排序（FTS5 BM25 暂缓，
         rerank 留恒等接口位）；打分叠加时效衰减（半衰期 30 天）
       - user_profiles 记忆画像 + 偏好画像（见§四）
  → ③ 工具节点（tool calling，模型决定调不调）：
       倾诉轮结构性跳过（不查数据，省 tool-plan 的 LLM 空转）
  ...
  → ④ 人设扮演生成：**酒馆式人设卡**（用户设立一次、持久记住、每轮扮演，见§四）
       ...
       流式（/chat/stream）：内容逐段经 on_delta 推给 SSE 端点，工具轮内容不外发
  → ⑥ 记忆写回：记住用户说了什么、发了什么——从本轮对话抽取偏好/事实/事件，
       实时写入 L1 原子记忆（langmem，见§四）

降级链（Kimi 抖动只降级不 500）：路由挂了默认倾诉轮；工具决策挂了当轮无工具；
生成挂了如实 502。每轮经任务层落 task_runs（treehole_chat），延迟/降级/失败可查。
```
