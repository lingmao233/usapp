...
## BUG-021 红茶改一次火腿后识别永久错乱：名字纠正双向震荡

- **日期**：2026-08-22
- **环境**：本机 dev + 生产（代码问题，两端同在；线上旧版未部署故生产未爆）
- **现象**：用户把一次识别出的「红茶」手动改成「火腿」，之后每次拍红茶都被识别成火腿；用户三次「改回来」无效，甚至出现识别红茶变火腿、识别火腿变红茶来回跳。
- **根因**：`update_calorie_item` 改名时无条件 INSERT 纠正行（`calorie_name_corrections`），且 `_apply_name_correction` 按 recognized_name 精确匹配取最新一条生效。误改写入 (红茶→火腿) 后：「改回来」只是新增 (火腿→红茶) 反向行，正向行永不删除 → 双向行并存，识别结果按「最新一条」来回震荡。纠正机制本身没有撤销语义、没有方向约束、items 里也不留「模型原始识别名」，无法判断用户是在纠正还是在撤销。
- **修复**：
  - 识别/改名时在 item 里留底 `raw_name`（视觉模型原始输出或最早录入名；识别时纠正生效才落，改名时 setdefault）
  - `_record_name_correction`：改回 raw_name = 撤销——删除正向行、不插入反向行；其他改名先删反向行 (new→key) 再插入，保证任何时刻一个名字对只有一个方向；被纠正过的条目再改名，纠正记在 raw_name 上（模型下次还输出这个名字）
  - 存量脏数据：`scripts/cleanup_pollution.py`（dry-run 默认）按「生效纠正图找环、删环上名字全部行（含被压住的旧行）」清理——只删环行会让火腿→茶 这类被压住的旧行复活成新错误
- **验证**：`test_calorie_fixes.py` 防震荡三用例（改回撤销/记在 raw_name/反向行清理）+ 真实本地库 dry-run 精准命中 4 行脏数据后 --apply 清掉、复跑幂等。pytest 242/242，smoke 62 断言。
- **预防**：**「学习型」纠正表必须有撤销语义与方向约束**——用户改回原值是撤销不是新纠正；插入任何 (A→B) 前先删 (B→A)。前端管理面板（我的识别纠正记录）必须真的挂载渲染（本次发现组件写了从未 render，用户根本没有自助清理入口）。

## BUG-022 红茶 50g 算出 147 kcal：联网错值只做范围校验就落全局库

- **日期**：2026-08-22
- **环境**：本机 dev + 生产（staging 是全局共享表，一行错值污染所有账号）
- **现象**：红茶 50g 给出 147 kcal（红茶本身约 1-2 kcal/100g）。50g 火腿恰好 ≈150 kcal，起初误以为与 BUG-021 同源，取证是独立故障。
- **根因**：正式表查不到「红茶」→ 后台 `web_search_food("红茶")` 联网返回 294 kcal/100g（疑似把 kJ 当 kcal：294kJ≈70kcal，且即便是含糖茶也偏高）→ 唯一校验是 `0<x≤1000`（`ai/__init__.py` web_search_food）→ 落 `food_nutrition_staging` 且直接 `verified=1` → 294×50/100=147。staging 无任何删除/编辑入口，错值无法治理。
- **修复**：
  - `nutrition.sanitize_web_value` 入库前清洗：①饮品品类先验——名字以 茶/水/汁/汤/咖啡/可乐 等结尾（结尾词判断，茶肠/茶树菇/水果不会误伤）且超先验上限（饮品 80、奶茶类 150）→ 拒绝落库；②kJ 误报折算——联网值 ≈ 模型隐含单价 × 4.184（±20%）→ 判为单位误报，除以 4.184 后再过先验。识别热路径把模型隐含单价随 missed 带给后台回填做参照；改名路径无参照只走先验
  - 曾实现「与模型估值差 >10 倍即拒」的对称交叉校验，实测误伤正确联网值（模型对未知食物高估/低估都常见：能量棒估 1、果汁估 565），降级为 kJ 折算参照——拒绝判定只交给品类先验，非饮品错值靠管理面板治理
  - staging 治理闭环：`GET/PATCH/DELETE /api/nutrition/staging`（软删 + updated_by/updated_at 审计 + 认可记录清理），软删行匹配/回填/认可即刻跳过、再添加同名以新值复活；前端「共建食物库管理」面板（查/改值/删/核验标记）
  - 存量错值：`cleanup_pollution.py` 按 source=web + 品类先验自动筛出并删（本地库命中红茶=294 一行）
- **验证**：`test_calorie_fixes.py`（先验拒绝/结尾词不误伤/kJ 折算/全链路错值不落库、合理值照常落并挂升级提示/管理 CRUD/软删摘掉升级提示）；本地库清理后复跑幂等。pytest 242/242，smoke 62 断言。
- **预防**：**外部数据入库前必须有「品类常识」校验，范围校验（0<x≤1000）挡不住单位误报和错配食材**；全局共享的信任管线必须配治理入口（可删可改），否则一行错值永久污染所有人。

## BUG-023 更新估值等 10 分钟只提示一个：无推送 + 提示条件过窄

- **日期**：2026-08-22
- **环境**：本机 dev + 生产
- **现象**：识别入账后干等 10 分钟，「有查表数据，更新为 X kcal」只出现过一个，其余条目一直没有提示。
- **根因**（多因叠加）：① 前端无任何轮询/推送——列表只在挂载/切日/操作后拉取，联网回填 20-25s 落库后界面纹丝不动，提示只等用户下次手动刷新；② 提示只挂 `source=model` 且有 grams 的条目，查表命中的永远不提示；③ 联网失败静默（返回 None 记 success），失败的菜名永远无 staging 行；④ staging 按 (name, brand) 精确匹配，用户改过名后断链。
- **修复**：
  - `services/events.py` 进程内 SSE 总线（跨线程投递经 loop.call_soon_threadsafe，30s 心跳，慢消费者丢事件）：`backfill_food_web` 入库成功即向该账号 publish `staging_ready`；staging 治理动作 `publish_all`
  - `GET /api/calories/events?account_id=`（StreamingResponse，text/event-stream，X-Accel-Buffering: no 防代理缓冲）；前端 Calories 页挂 EventSource，收到消息自动 load()——升级提示自动出现，无需新依赖（符合白名单）
